...
function defaultPrivateItems(userId) {
  if (userId === 'tester') {
    return [
...
function cleanPrivateText(value, maxLength) {
    ...
    users: [
      {
        ...
        privateItems: defaultPrivateItems('owner'),
      },
      {
        id: 'tester', username: 'yerim-tester', label: '과제용 테스트 계정', isOwner: false,
        webAuthnUserID: randomId(32), createdAt: nowISO(),
        privateItems: defaultPrivateItems('tester'),
      },
    ],
...
async function handleApi(req, res, url, db) {
  ...
  if (req.method === 'POST' && url.pathname === '/api/private/items') {
    if (!auth) return json(res,401,{error:'AUTH_REQUIRED'});
    const body = await readBody(req);
    const title = cleanPrivateText(body.title, 60);
    const text = cleanPrivateText(body.body, 500);
    if (!title || !text) return json(res,400,{error:'INVALID_PRIVATE_ITEM',message:'제목과 내용을 모두 입력하세요.'});
    const item = { id:`${auth.user.id}-${randomId(8)}`, title, body:text };
    auth.user.privateItems.push(item);
    audit(db,'private_item_added',{account:auth.user.id,itemId:item.id});
    saveDB(db);
    return json(res,201,{ok:true,item,count:auth.user.privateItems.length});
  }

  const privateItem = url.pathname.match(/^\/api\/private\/items\/([^/]+)$/);
  if (req.method === 'PATCH' && privateItem) {
    if (!auth) return json(res,401,{error:'AUTH_REQUIRED'});
    const itemId = decodeURIComponent(privateItem[1]);
    const item = auth.user.privateItems.find(x => x.id === itemId);
    if (!item) return json(res,404,{error:'PRIVATE_ITEM_NOT_FOUND'});
    const body = await readBody(req);
    const title = cleanPrivateText(body.title, 60);
    const text = cleanPrivateText(body.body, 500);
    if (!title || !text) return json(res,400,{error:'INVALID_PRIVATE_ITEM',message:'제목과 내용을 모두 입력하세요.'});
    item.title = title;
    item.body = text;
    audit(db,'private_item_updated',{account:auth.user.id,itemId:item.id});
    saveDB(db);
    return json(res,200,{ok:true,item});
  }

  if (req.method === 'POST' && url.pathname === '/api/private/reset') {
    if (!auth) return json(res,401,{error:'AUTH_REQUIRED'});
    auth.user.privateItems = defaultPrivateItems(auth.user.id);
    audit(db,'private_items_reset',{account:auth.user.id,count:auth.user.privateItems.length});
    saveDB(db);
    return json(res,200,{ok:true,items:auth.user.privateItems,count:auth.user.privateItems.length});
  }
